...
- Workspace access review export is owner-only, selected-workspace scoped, and available as JSON or CSV from `GET /api/studio/settings/access-review`; `POST /api/studio/settings/access-review` records an audit-backed evidence hash for the current review snapshot, and `GET /api/studio/settings/access-review/attestations` lists prior review attestations as JSON or CSV.
