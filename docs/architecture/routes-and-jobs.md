...
- Workspace export is owner-only at `GET /api/studio/workspaces/[workspaceId]/export`; JSON and CSV export modes include the same access-review freshness state and summary deltas so offboarding packages show whether governance evidence is current before archive/decommission decisions.
