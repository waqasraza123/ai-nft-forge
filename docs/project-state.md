...
- `packages/shared` now centralizes worker env validation, auth request/response schemas, storage env parsing, reusable object-storage helpers, source asset upload contracts, generation request contracts, generation-backend health/readiness contracts, generated asset contracts including moderation state, collection draft, publication, public metadata contracts, commerce reservation and checkout contracts, onchain deployment/mint request and summary contracts, reconciliation issue/run contracts including onchain drift issue kinds, studio settings contracts, and queue payload definitions.
...
- The public storefront now also exposes item-level reservations and a hosted manual checkout page for live releases, with provider-mode gating, lazy reservation expiry, and sold-count/storefront updates applied only after checkout completion.
...
- The first commerce slice should reserve specific published collection items before checkout, keep the published snapshot immutable, and use a provider boundary with `manual` and `disabled` modes so self-host deployments can validate commerce flows without depending on an external payment processor yet.
...
- The studio and public surfaces now expose per-asset generation history, owner-scoped collection drafts, durable studio settings, storefront merchandising, preset-driven public presentation, published collection launch pages, and item-level manual checkout, but multi-brand administration, external payment processors, and richer cross-release merchandising are still deferred.
