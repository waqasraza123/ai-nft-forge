...
AI NFT Forge is planned as a self-hosted, white-label product for turning client photos into collectible-style art variants, curating final assets, publishing branded collection pages, and later minting onchain. The repository now contains the completed Phase 1 foundation, the first twenty-one Phase 2 slices, the first six Phase 3 slices, and the completed current Phase 4 scope: storage-backed source asset intake, queue-backed generation orchestration, durable generated-output persistence, the first external-backend plus protected-download boundary, the first interactive browser workflow for upload, dispatch, polling, and retrieval, a concrete standalone generation backend service behind the worker HTTP contract, the first model-backed ComfyUI provider inside that backend, provider-aware ops diagnostics plus backend readiness checks around that real model path, richer studio-visible generation metadata plus explicit failed-request retry ergonomics, authenticated ops queue depth plus owner-scoped generation activity and retry controls, fuller per-asset generation history plus archived-run comparison inside the studio, rolling owner-scoped ops metrics plus synthesized alert diagnostics beyond point-in-time queue depth, browser-level smoke coverage for the protected studio and ops workflows, persisted multi-day ops observability captures plus durable alert-delivery history, worker-owned capture scheduling plus operator-visible automation status on `/ops`, generic outbound webhook delivery for persisted ops alerts, explicit owner-scoped acknowledgment controls for active persisted alerts on `/ops`, owner-scoped bounded mute policy for persisted alert delivery on `/ops`, owner-scoped webhook routing preferences for persisted ops alerts on `/ops`, owner-scoped webhook delivery schedules for persisted ops alerts on `/ops`, owner-scoped webhook escalation policy for repeated unacknowledged persisted ops alerts on `/ops`, owner-scoped collection draft creation, metadata editing, ordered curated generated-asset selection, and review-ready status management on `/studio/collections`, published collection snapshots plus live public collection delivery on `/brands/[brandSlug]/collections/[collectionSlug]`, durable owner-scoped workspace plus brand settings on `/studio/settings` that now drive publication defaults, a branded public landing route on `/brands/[brandSlug]` that presents published collection previews from saved brand identity, owner-scoped publication merchandising controls for featured release and display order, durable storefront landing copy in studio settings that now drives public brand-route headline, description, and spotlight labeling, public collection metadata manifests plus per-edition metadata records on `/brands/[brandSlug]/collections/[collectionSlug]/metadata`, publication-time durable public asset promotion for published collection media with legacy signed-url fallback, and published-snapshot-backed contract manifests plus token-uri routes for initial chain publishing support. Minting is still not implemented.
...
- `apps/web` now also exposes `POST/DELETE/PATCH /api/studio/collections/[collectionDraftId]/publish`, `GET/PUT /api/studio/settings`, a protected `/studio/settings` surface for durable workspace identity, brand identity, and public landing-copy management, a branded public `/brands/[brandSlug]` landing route, a live `/brands/[brandSlug]/collections/[collectionSlug]` page that resolves published collection snapshots and now prefers durable public published-asset URLs for public viewing, public metadata JSON at `/brands/[brandSlug]/collections/[collectionSlug]/metadata` plus `/brands/[brandSlug]/collections/[collectionSlug]/metadata/[editionNumber]`, and public contract/token-uri JSON at `/brands/[brandSlug]/collections/[collectionSlug]/contract` plus `/brands/[brandSlug]/collections/[collectionSlug]/token-uri/[tokenId]`.
...
- `packages/shared` now centralizes worker env validation, auth request/response schemas, storage env parsing, reusable object-storage helpers, source asset upload contracts, generation request contracts, generation-backend health/readiness contracts, generated asset contracts, collection draft, publication, and public metadata contracts, studio settings contracts, and queue payload definitions.
- `packages/contracts` now provides deterministic collection-contract helper utilities for Base Sepolia/Base publication metadata, route/url derivation, contract naming, and token-uri path construction.
- `packages/database` now contains the Prisma schema, initial SQL migration, repository helpers, transaction helper, PostgreSQL client boundary, and the `SourceAsset`, `GenerationRequest`, `GeneratedAsset`, `CollectionDraft`, `CollectionDraftItem`, `PublishedCollection`, `PublishedCollectionItem` with public-asset references, `Workspace`, and `Brand` models and repositories used by the current app surface.
...
- Public contract manifests and token-uri routes should also resolve strictly from immutable published collection snapshots so initial chain-publishing inputs stay aligned with the already-published public release boundary.
...
- Contract deployment, final onchain publication, and minting
...
- The ops surface now exposes backend liveness/readiness plus authenticated queue depth, rolling owner-scoped metrics, persisted observability history, active-alert acknowledgment and mute controls, owner-scoped webhook routing, delivery-schedule, and escalation policy, durable multi-channel alert-delivery records, capture-automation health, synthesized alerts, and owner-scoped retry controls, but cross-user/workspace controls are still deferred.
- The studio and public surfaces now expose per-asset generation history, owner-scoped collection drafts, durable studio settings, publication merchandising, saved storefront landing copy, public collection publication, and a branded public release directory, but multi-brand administration, live storefront commerce, and richer cross-release merchandising are still deferred.
- New publications now use durable public asset URLs for collection and metadata image delivery, but older publications still require republish to gain public copies, and onchain contract publication remains deferred.
