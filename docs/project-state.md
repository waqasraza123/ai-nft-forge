...
- `packages/database` now contains the Prisma schema, initial SQL migration, repository helpers, transaction helper, PostgreSQL client boundary, and the `SourceAsset`, `GenerationRequest`, `GeneratedAsset` with owner-scoped moderation state, `CollectionDraft`, `CollectionDraftItem`, `PublishedCollection`, `PublishedCollectionItem` with public-asset references, recorded contract deployment fields, `PublishedCollectionMint`, `Workspace`, and `Brand` models and repositories used by the current app surface.
...
- `.env.example` now documents the current local and self-host runtime environment shape, including reconciliation automation, compose-facing service ports, and generation-backend readiness timeout tuning.
...
- The worker and `/ops` surface now also support owner-scoped reconciliation with persisted runs and issues, Redis-leased scheduled execution, manual run/repair/ignore controls, repair-time public-asset recopies and draft downgrades, and freshness/critical-issue warnings for operators.
...
- Onchain deployment and minting should remain owner-signed control-plane actions: the repo may prepare deploy/mint intents and record submitted transactions, but it must not require server-held private keys.
...
- The repo now supports owner-signed deployment and mint intent preparation plus recorded onchain deployment and mint ledgers, but it still lacks a polished wallet UX, automated transaction submission, and chain-state reconciliation beyond the explicitly recorded control-plane data.
