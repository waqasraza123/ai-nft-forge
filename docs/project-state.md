...
- `packages/shared` now centralizes worker env validation, auth request/response schemas, storage env parsing, reusable object-storage helpers, source asset upload contracts, generation request contracts, generation-backend health/readiness contracts, generated asset contracts including moderation state, collection draft, publication, public metadata contracts, commerce reservation, checkout, dashboard, fulfillment automation callback/retry contracts including explicit publication pricing fields plus Stripe and webhook-fulfillment env parsing, onchain deployment/mint request and summary contracts, reconciliation issue/run contracts including onchain drift issue kinds, studio settings contracts including workspace role escalation and ownership-transfer review, ops audit query/response contracts, and queue payload definitions.
...
- Workspace access should resolve at the web boundary from the signed-in actor to one effective owner workspace so existing service and repository layers can stay owner-anchored while memberships expand access safely.
...
- Audit-oriented administration should currently stay workspace-scoped: operators and owners may review and export activity for their effective workspace, but broader multi-workspace or fleet-level admin views remain deferred until the one-effective-workspace constraint changes.
- Workspace invitations should stay wallet-address-driven and owner-created, auto-accept on first authenticated sign-in for actors without an existing effective workspace, and write durable membership lifecycle entries into the workspace audit log while preserving the current one-effective-workspace constraint.
...
- Broader multi-workspace audit-oriented administration
...
- The ops surface now exposes backend liveness/readiness plus authenticated queue depth, rolling workspace-scoped metrics, persisted observability history, active-alert acknowledgment and mute controls, owner-scoped webhook routing, delivery-schedule, escalation policy, reconciliation automation health, persisted reconciliation issues, and workspace activity audit review/export, but broader audit-grade multi-workspace administration is still deferred.
- The studio and public surfaces now expose per-asset generation history, workspace-scoped collection drafts, multi-brand studio settings, explicit publication target selection, storefront merchandising, preset-driven public presentation, published collection launch pages, item-level manual or Stripe-hosted checkout, owner-side commerce administration, webhook fulfillment automation, brand-scoped commerce partitions, owner/operator workspace permissions, and wallet-address invitation plus member-lifecycle audit history, but broader role variants are still deferred.
