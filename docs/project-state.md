...
- `packages/database` now contains the Prisma schema, initial SQL migration, repository helpers, transaction helper, PostgreSQL client boundary, and the `SourceAsset`, `GenerationRequest`, `GeneratedAsset` with owner-scoped moderation state, `CollectionDraft`, `CollectionDraftItem`, `PublishedCollection`, `PublishedCollectionItem` with public-asset references, recorded contract deployment fields, `PublishedCollectionMint`, `PublishedCollectionReservation`, `CommerceCheckoutSession` with durable fulfillment automation fields, `Workspace`, and `Brand` models and repositories used by the current app surface.
...
- Collection authoring should stay owner-scoped and generated-asset-driven until fuller multi-operator permissions exist; public publication should consume immutable published snapshots rather than mutable drafts directly, while saved owner-scoped studio settings now provide multiple brand publication targets under one workspace without reopening workspace-role complexity.
...
- Multi-operator/workspace-role permissions
...
- The studio and public surfaces now expose per-asset generation history, owner-scoped collection drafts, multi-brand studio settings, explicit publication target selection, storefront merchandising, preset-driven public presentation, published collection launch pages, item-level manual or Stripe-hosted checkout, owner-side commerce administration, and webhook fulfillment automation, but multi-operator permissions are still deferred.
