...
- `.env.example` now documents the current local and self-host runtime environment shape, including reconciliation automation, Base/Base Sepolia RPC endpoints for onchain verification, compose-facing service ports, and generation-backend readiness timeout tuning.
...
- The studio onchain flow now includes a real browser-wallet deployment and mint path with server-side receipt verification, but broader wallet UX and Base Account integration remain deferred.
...
- The repo now supports wallet-driven owner-signed deployment and mint flows plus recorded onchain deployment and mint ledgers, but it still lacks Base Account integration, richer wallet UX polish, and chain-state reconciliation beyond the explicitly verified transaction-recording path.
