...
- Owner-only workspace access review export plus shared deterministic evidence hashes, current/changed attestation freshness with summary deltas, and audit-backed review attestation history for governance handoff as JSON or CSV
- Owner-only workspace export and offboarding packets that carry access-review freshness evidence alongside archive-readiness, lifecycle, commerce, and ops state, with stale or missing reviews blocking retained decommission schedule, notice, and execution steps
...
- `/ops/audit` can filter and export access, ownership-transfer, workspace lifecycle, and workspace policy audit categories from the selected workspace, including structured decommission metadata plus previous/current policy metadata columns
